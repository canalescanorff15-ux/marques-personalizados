...
const brand=process.env.NEXT_PUBLIC_SITE_NAME||'Merlin Encantos em Papel';const whatsapp=process.env.NEXT_PUBLIC_WHATSAPP_NUMBER||'';const instagram=process.env.NEXT_PUBLIC_INSTAGRAM_URL||'';const location=process.env.NEXT_PUBLIC_LOCATION||'Santa Inês - MA';const email=process.env.NEXT_PUBLIC_CONTACT_EMAIL||'';
await sql`INSERT INTO site_settings (id,brand_name,whatsapp_number,instagram_url,location,contact_email,seo_title) VALUES (1,${brand},${whatsapp},${instagram},${location},${email},${`${brand} | Papelaria Personalizada Premium`}) ON CONFLICT (id) DO UPDATE SET brand_name=CASE WHEN site_settings.brand_name IN ('Marques Papelaria','K&F Papelaria Criativa') THEN EXCLUDED.brand_name ELSE site_settings.brand_name END,whatsapp_number=CASE WHEN site_settings.whatsapp_number='' THEN EXCLUDED.whatsapp_number ELSE site_settings.whatsapp_number END,instagram_url=CASE WHEN site_settings.instagram_url='' THEN EXCLUDED.instagram_url ELSE site_settings.instagram_url END,location=CASE WHEN site_settings.location='Santa Inês - MA' THEN EXCLUDED.location ELSE site_settings.location END,contact_email=CASE WHEN site_settings.contact_email='' THEN EXCLUDED.contact_email ELSE site_settings.contact_email END`;
