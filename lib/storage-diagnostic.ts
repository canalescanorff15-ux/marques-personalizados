...
export async function runStorageDiagnostic(timeoutMs=5000):Promise<StorageDiagnosticResult>{
  ...
  try{
    ...
    const head=await stage('head',async signal=>{const res=await client.send(new HeadObjectCommand({Bucket:c.bucket,Key:key}),{abortSignal:signal});if(Number(res.ContentLength)!==body.length||String(res.Metadata?.['probe-token']||'')!==token)throw new Error('STORAGE_PROBE_HEAD_MISMATCH');},stages,timeoutMs);
    ...
    const list=await stage('list',async signal=>{const res=await client.send(new ListObjectsV2Command({Bucket:c.bucket,Prefix:key,MaxKeys:2}),{abortSignal:signal});if(!(res.Contents||[]).some(item=>item.Key===key))throw new Error('STORAGE_PROBE_LIST_MISMATCH');},stages,timeoutMs);
    ...
    const confirm=await stage('confirm_delete',async signal=>{try{await client.send(new HeadObjectCommand({Bucket:c.bucket,Key:key}),{abortSignal:signal});throw new Error('STORAGE_PROBE_DELETE_NOT_CONFIRMED');}catch(error){if(missingObjectError(error))return;throw error;}},stages,timeoutMs);
