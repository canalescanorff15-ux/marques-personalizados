...
export default function ProductOrderPlanner({product}:{product:Product}){
  ...
  return <section className="product-order-planner" id="planejar-pedido" aria-labelledby="order-planner-title">
    <div className="product-order-planner-head"><span><Calculator size={16}/> SIMULADOR INICIAL</span><h3 id="order-planner-title">Escolha a quantidade antes de adicionar.</h3><p>O cálculo usa somente o preço inicial cadastrado. Arte, acabamento, urgência e frete são confirmados no orçamento.</p></div>
    ...
    <button type="button" className={`btn btn-primary product-order-add ${added?'is-added':''}`} onClick={add}>{added?<Check size={17}/>:<ShoppingBag size={17}/>} {added?'Atualizar quantidade na lista':'Adicionar à minha lista'}</button>
  </section>;
}
