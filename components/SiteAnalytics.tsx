...
export function trackSessionEvent(event:string,path:string,key:string){if(!isCanonicalTelemetryOrigin())return;try{if(sessionStorage.getItem(key))return;sessionStorage.setItem(key,'1');}catch{}send(event,path);}
export default function SiteAnalytics(){
  const pathname=usePathname();
  useEffect(()=>{
    const path=pathname||'/';if(path.startsWith('/admin')||!isCanonicalTelemetryOrigin())return;
  ...
  useEffect(()=>{
    if((pathname||'/').startsWith('/admin')||!isCanonicalTelemetryOrigin())return;
