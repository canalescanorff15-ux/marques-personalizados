...
export default async function LinksPage(){
 ...
 return <main className="bio-page"><section className="bio-shell">
  <div className="bio-brand">{settings.logo_url?<SafeImage src={settings.logo_url} alt={settings.brand_name}/>:<span className="bio-monogram">{settings.brand_initial||'M'}</span>}<div className="eyebrow">TOPOS DE BOLO PERSONALIZADOS</div><h1>{settings.brand_name}</h1><p>Do Essencial ao Elite com shaker + acetato. Produção sob encomenda e personalizada para o seu tema.</p><SocialLinks settings={settings} showLabels/></div>
